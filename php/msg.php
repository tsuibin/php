...
function viewmsg($i){
	...
	//print_r($amsg);
	$msgnumber=count($cmsg) ;//一共有多少条信息
	global $msgnumber;
	/**
	echo "<table border='1'>";
	echo "<tr><td>name</td><td>".$amsg[$i][0]."</td></tr>";
	echo "<tr><td>title</td><td>".$amsg[$i][1]."</td></tr>";
	echo "<tr><td>message</td><td>".$amsg[$i][2]."</td></tr>";
	echo "<tr><td>time</td><td>".$amsg[$i][3]."</td></tr>";
	echo "<tr><td>imageurl</td><td>".$amsg[$i][4]."</td></tr>";
	echo "</table>";
	/**/
}
...
function ccc($ps){
	...
	$ps=$ps-1;
	global $msgnumber;
	echo "第".$page."页 总共 $msgnumber 条消息 每页显示{$ps}条";
}
//分割文本内容
