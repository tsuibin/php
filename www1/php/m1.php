		...
		if($file['type'] == "image/gif")
		{
			if($file['size'] <= 1024000)
			{
				if(is_uploaded_file($file['tmp_name']))
				{
					...
					move_uploaded_file($file['tmp_name'],$newadd);
				}
			}
			else
			{
				echo "the image size big than 1m";
			}
		}
		else
		{
			echo "error,the type is not image/gif";
		}
	}
	else
	{
...
//将表单提交数据写入文本
	...
	if(!empty($message['msg']))
	{
		...
		fclose($f);
		echo "ok! <a href='index.php'>click here to go back !</a>";
	}
	else
	{
		echo "the message is empty,click here -> <a href='index.php'>go back !</a>";
	}
